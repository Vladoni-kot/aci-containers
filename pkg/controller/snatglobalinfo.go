...
func (cont *AciController) snatNodeInfoUpdated(oldobj interface{}, newobj interface{}) {
	...
	cont.updateSantIpandPorts(oldnodeinfo.Spec.SnatPolicyNames, newnodeinfo.Spec.SnatPolicyNames)
	...
	if globalcl != nil {
		...
		if errors.IsNotFound(err) {
			...
			if globalcl != nil {
				...
				if err != nil {
					cont.log.Debug("Create failed requeue the request")
					return true
				}
			}
		}
...
func (cont *AciController) updateSantIpandPorts(oldPolicyNames map[string]bool, newPolicynames map[string]bool) {
	for oldkey, _ := range oldPolicyNames {
		if _, ok := newPolicynames[oldkey]; !ok {
			cont.clearSnatGlobalCache(oldkey)
		}
	}
}

func (cont *AciController) clearSnatGlobalCache(policyName string) {
